...
async function authorizeWithSupabase(loginId:string, password:string) {
  ...
  const { data, error } = await supabaseAdmin
    .from("koryo_classes").select("id,login_id,password_hash,name").eq("login_id",loginId).maybeSingle();
...
export const authOptions: NextAuthOptions = {
  providers: [CredentialsProvider({
    name:"credentials",
    credentials:{ loginId:{label:"ログインID",type:"text"}, password:{label:"パスワード",type:"password"} },
  ...
  callbacks:{
    async jwt({ token, user }) { if (user) { token.id=user.id; token.classId=user.classId; } return token; },
    async session({ session, token }) { if (session.user) { session.user.id=token.id; session.user.classId=token.classId; } return session; },
  },
